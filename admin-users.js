...
export async function showAdminUsers() {
  ...
  if (!user || String(user.id) !== JOBPILOT_ADMIN_ID) { alert("You do not have permission to view users."); return; }
  ...
  pageContent.innerHTML = `<div class="page-header"><div><h2>👥 Users</h2><p>Search users by name, phone number or email.</p></div></div><div class="panel" style="margin-bottom:16px;"><input id="adminUserSearch" class="search-input" type="search" placeholder="Search by name, phone or email..." autocomplete="off"></div><div id="adminUsersList"><div class="panel"><p>Loading users...</p></div></div>`;
  const container = document.getElementById("adminUsersList");
  const searchInput = document.getElementById("adminUserSearch");
  if (!container) return;
  const loadUsers = async () => { const { data, error } = await supabase.functions.invoke("admin-users", { body: { action: "list" } }); if (error) { container.innerHTML = `<div class="panel"><h3>Could not load users</h3><p>${escapeHtml(error.message || "Unable to load users.")}</p></div>`; return []; } return Array.isArray(data?.users) ? data.users : []; };
  const users = await loadUsers();
  if (!users.length) { container.innerHTML = `<div class="panel"><h3>No users found</h3><p>There are currently no registered JobPilot accounts.</p></div>`; return; }
  ...
  const showEditForm = (user) => {
    ...
    const details = [...container.querySelectorAll("details")].find(el => el.querySelector(`[data-edit-user="${CSS.escape(user.id)}"]`)); if (!details) return;
    const form = document.createElement("div"); form.setAttribute("data-user-form", user.id); form.style.cssText = "margin-top:16px;padding:16px;border-top:1px solid var(--border-color,#ddd);";
    ...
    details.appendChild(form); form.querySelector("[data-cancel-edit]").addEventListener("click", () => form.remove()); form.querySelector("[data-save-edit]").addEventListener("click", async () => { const get = field => form.querySelector(`[data-field="${field}"]`).value.trim(); const payload = { action: "update", user_id: user.id, name: get("name"), phone: get("phone"), email: get("email"), business_name: get("business_name") }; const save = form.querySelector("[data-save-edit]"); save.disabled = true; save.textContent = "Saving..."; const { data, error } = await supabase.functions.invoke("admin-users", { body: payload }); if (error) { alert(`Could not update user: ${error.message || "Unknown error"}`); save.disabled = false; save.textContent = "Save changes"; return; } Object.assign(user, data.user || payload); renderUsers(searchInput?.value || ""); });
  };
