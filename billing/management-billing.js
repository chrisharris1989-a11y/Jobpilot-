  ...
  async function renderManagementBillingPage() {
      ...
      <div class="content-grid">
        ...
        <div class="panel" id="jobpilot-management-sms-settings-card">
          ...
          <div id="jobpilot-auto-sms-options" style="display:none;margin-top:12px;padding:14px;border-radius:8px;background:#fff;border:1px solid #e2e8f0;">
            <div style="font-weight:600;">Choose automated messages</div>
            <div style="margin-top:4px;font-size:13px;color:#64748b;">Select which messages JobPilot is allowed to send automatically.</div>
            <div id="jobpilot-auto-sms-checklist" style="margin-top:12px;display:grid;gap:8px;"></div>
            <div id="jobpilot-auto-sms-save-message" style="margin-top:10px;font-size:13px;color:#64748b;"></div>
          </div>
    ...
    function renderAutomationChecklist() {
      ...
      SMS_AUTOMATIONS.forEach(item => {
        ...
        autoSmsChecklist.appendChild(label);
      });
    }
    ...
    async function paint(planName, entitlement = null) {
      ...
      if (planChoices.length) {
        ...
        changeLabel.style.width = "100%"; changeLabel.style.marginBottom = "4px"; changeLabel.style.fontWeight = "600"; changeLabel.textContent = isTest ? "Change test plan" : "Change plan";
        ...
        planChoices.forEach(name => {
          ...
          option.addEventListener("click", async () => {
            if (isTest) { const result = await switchTestPlan(name, message); if (result) await paint(String(result.plan || name).toLowerCase(), null); return; }
            const confirmed = window.confirm(`Are you sure you want to change your plan to ${PLANS[name].label} at ${formatPrice(PLANS[name])}?`);
            if (confirmed) openBilling("change_plan", name, message);
          });
          options.appendChild(option);
        });
      } else {
        const current = document.createElement("div"); current.style.marginTop = "12px"; current.style.fontSize = "13px"; current.textContent = "You are on the highest available plan."; options.appendChild(current);
      }
      ...
      if (!isTest && ["active", "trialing", "past_due", "canceled"].includes(status)) {
        ...
        manage.addEventListener("click", () => openBilling("portal", planName, message));
    ...
    if (!isTest) {
      try {
        ...
        if (!error && data?.length) {
          ...
          if (entitlement.cancel_at_period_end && entitlement.current_period_end) { message.textContent = `Your subscription is scheduled to end on ${new Date(entitlement.current_period_end).toLocaleDateString()}.`; message.style.color = "#92400e"; }
        }
