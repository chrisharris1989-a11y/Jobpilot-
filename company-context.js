...
const PLAN_OPTIONS = [
  { id: "solo", label: "Solo", description: "For one person", users: 1 },
  { id: "team", label: "Team", description: "Up to 10 users", users: 10 },
  { id: "business", label: "Business", description: "Up to 25 users", users: 25 },
  { id: "pro", label: "Pro", description: "Up to 50 users", users: 50 }
];
...
export async function ensureCompanyContext() {
  ...
  if (memberError) {
    console.error("Company membership:", memberError);
    throw memberError;
  }

  if (!member) {
    return showCompanyOnboarding(user);
  }
  ...
  if (!companyData) {
    throw new Error("Your JobPilot company could not be found.");
  }
...
async function showCompanyOnboarding(user) {
    ...
    <div style="position:fixed;inset:0;z-index:99999;background:#f8fafc;display:flex;align-items:center;justify-content:center;padding:24px;font-family:inherit;overflow:auto;">
      <div style="width:100%;max-width:680px;background:#fff;border:1px solid #e5e7eb;border-radius:18px;padding:32px;box-shadow:0 20px 50px rgba(15,23,42,.12);">
        <div style="margin-bottom:26px;">
          <div style="font-size:28px;font-weight:800;color:#111827;">Set up your JobPilot workspace</div>
          <p style="margin:8px 0 0;color:#64748b;">Choose your starting plan and business name. You can manage your team later from Settings.</p>
        </div>
        ...
        <div id="jobpilot-plan-options" style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;">
          ${PLAN_OPTIONS.map((plan, index) => `
            <button type="button" data-plan="${plan.id}" style="text-align:left;padding:16px;border:2px solid ${index === 0 ? "#2563eb" : "#e2e8f0"};border-radius:12px;background:#fff;cursor:pointer;font:inherit;">
              <div style="font-weight:700;color:#111827;">${plan.label}</div>
              <div style="font-size:13px;color:#64748b;margin-top:4px;">${plan.description}</div>
            </button>
          `).join("")}
        </div>
        ...
        <button id="jobpilot-company-submit" type="button" style="width:100%;margin-top:20px;padding:13px 16px;border:0;border-radius:10px;background:#2563eb;color:#fff;font-weight:700;font-size:15px;cursor:pointer;">
  ...
  root.querySelector("#jobpilot-company-submit").addEventListener("click", async () => {
    ...
    submit.textContent = "Setting up your workspace...";
    errorBox.style.display = "none";

    const { data: companyId, error } = await supabase.rpc("create_my_company", {
    ...
    if (error) {
      ...
      errorBox.textContent = error.message || "We couldn't set up your workspace. Please try again.";
      errorBox.style.display = "block";
      submit.disabled = false;
      submit.textContent = "Continue to JobPilot";
      return;
    }
    ...
    if (companyError || membershipError) {
      console.error("Company context reload:", companyError || membershipError);
      errorBox.textContent = "Your workspace was created, but JobPilot could not load it. Please sign in again.";
      errorBox.style.display = "block";
      submit.disabled = false;
      submit.textContent = "Continue to JobPilot";
      return;
