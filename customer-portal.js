 ...
 * Customer accounts are linked to a JobPilot customer and company.
 */
export async function getCustomerPortalSession() {
  ...
  const { data, error } = await supabase
    ...
    .select('id, customer_id, company_id, status, customers:customer_id(id, name, email, phone, address, postcode)')
...
export async function updatePortalCustomer(customerId, updates) {
  const allowed = {};
  for (const key of ['name', 'email', 'phone', 'address', 'postcode']) {
