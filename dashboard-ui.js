...
async function updateTodaySnapshot() {
  ...
  // The sixth card replaces Today's Job Value with the monthly workload count.
  ...
  // Management sees the whole company's workload. A normal User sees only jobs assigned to them.
  ...
  todayJobsCard.querySelector("strong").textContent = String(todayJobs.length);
...
function openMonthCalendar(jobs, selectedDate = null) {
  ...
  overlay.innerHTML = `
    ...
    <div class="jobpilot-calendar-modal" role="dialog" aria-modal="true" aria-label="Planned work calendar">
      <div class="jobpilot-calendar-header"><div><h2 id="jobpilot-calendar-title"></h2><p>Planned work for this month</p></div><button type="button" class="jobpilot-calendar-close" aria-label="Close">×</button></div>
      <div class="jobpilot-calendar-nav"><button type="button" id="jobpilot-calendar-prev">‹</button><button type="button" id="jobpilot-calendar-today">Today</button><button type="button" id="jobpilot-calendar-next">›</button></div>
  ...
  if (!document.getElementById("jobpilot-calendar-style")) {
    ...
    style.textContent = `
      #jobpilot-month-calendar-overlay{position:fixed;inset:0;z-index:99999;display:flex;align-items:center;justify-content:center;padding:16px}.jobpilot-calendar-backdrop{position:absolute;inset:0;background:rgba(15,23,42,.48)}.jobpilot-calendar-modal{position:relative;width:min(920px,100%);max-height:92vh;overflow:auto;background:#fff;border-radius:18px;padding:20px;box-shadow:0 24px 80px rgba(15,23,42,.25)}.jobpilot-calendar-header{display:flex;justify-content:space-between;align-items:flex-start;gap:12px}.jobpilot-calendar-header h2{margin:0;font-size:24px}.jobpilot-calendar-header p{margin:4px 0 0;color:#64748b}.jobpilot-calendar-close{border:0;background:transparent;font-size:30px;line-height:1;cursor:pointer;padding:0 4px}.jobpilot-calendar-nav{display:flex;justify-content:space-between;align-items:center;margin:16px 0 10px}.jobpilot-calendar-nav button{border:1px solid #dbe2ea;background:#fff;border-radius:9px;padding:7px 13px;font-size:18px;cursor:pointer}.jobpilot-calendar-nav #jobpilot-calendar-today{font-size:14px}.jobpilot-calendar-grid{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));border-left:1px solid #e5e7eb;border-top:1px solid #e5e7eb}.jobpilot-calendar-weekday{font-weight:700;font-size:12px;text-align:center;padding:9px 4px;background:#f8fafc;border-right:1px solid #e5e7eb;border-bottom:1px solid #e5e7eb}.jobpilot-calendar-day{min-height:92px;padding:7px;border-right:1px solid #e5e7eb;border-bottom:1px solid #e5e7eb;background:#fff;cursor:pointer;text-align:left}.jobpilot-calendar-day.muted{background:#f8fafc;color:#94a3b8}.jobpilot-calendar-day.today{outline:2px solid #2563eb;outline-offset:-2px}.jobpilot-calendar-day.selected{background:#eff6ff}.jobpilot-calendar-number{font-weight:700;font-size:13px}.jobpilot-calendar-job{margin-top:5px;padding:4px 5px;border-radius:5px;background:#e0f2fe;font-size:11px;line-height:1.25;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.jobpilot-calendar-more{font-size:11px;color:#475569;margin-top:3px}.jobpilot-calendar-details{margin-top:14px;padding:12px;background:#f8fafc;border-radius:10px;min-height:20px}.jobpilot-calendar-details h3{margin:0 0 7px;font-size:15px}.jobpilot-calendar-details p{margin:4px 0;font-size:13px}@media(max-width:640px){.jobpilot-calendar-modal{padding:12px;border-radius:12px}.jobpilot-calendar-day{min-height:64px;padding:5px}.jobpilot-calendar-job{font-size:9px}.jobpilot-calendar-details{margin-top:10px}.jobpilot-calendar-header h2{font-size:20px}}`;
    document.head.appendChild(style);
  }
  ...
  document.addEventListener("keydown", function calendarEscape(event){ if (event.key === "Escape" && document.getElementById("jobpilot-month-calendar-overlay")) { close(); document.removeEventListener("keydown", calendarEscape); } });
  function renderDetails(iso) { const dayJobs = jobs.filter(job => job.scheduled_date === iso && String(job.status || "").toLowerCase() !== "cancelled"); const displayDate = new Date(`${iso}T12:00:00`).toLocaleDateString("en-GB", { weekday:"long", day:"numeric", month:"long", year:"numeric" }); details.innerHTML = dayJobs.length ? `<h3>${displayDate}</h3>${dayJobs.map(job => `<p><strong>${job.scheduled_time ? job.scheduled_time + " — " : ""}${String(job.title || "Job")}</strong>${job.notes ? ` — ${String(job.notes).replace(/[<>]/g, "")}` : ""}</p>`).join("")}` : `<p>No planned work on ${displayDate}.</p>`; }
  function render() {
    ...
    const first = new Date(year, month, 1); const firstDay = (first.getDay() + 6) % 7; const daysInMonth = new Date(year, month + 1, 0).getDate(); const previousDays = new Date(year, month, 0).getDate(); const totalCells = Math.ceil((firstDay + daysInMonth) / 7) * 7; const todayIso = getTodayDate();
    for (let cell = 0; cell < totalCells; cell++) { const dayNumber = cell - firstDay + 1; const date = new Date(year, month, dayNumber); const iso = `${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,"0")}-${String(date.getDate()).padStart(2,"0")}`; const inMonth = dayNumber >= 1 && dayNumber <= daysInMonth; const dayJobs = jobs.filter(job => job.scheduled_date === iso && String(job.status || "").toLowerCase() !== "cancelled"); const cellEl = document.createElement("button"); cellEl.type = "button"; cellEl.className = `jobpilot-calendar-day${inMonth ? "" : " muted"}${iso === todayIso ? " today" : ""}${iso === selectedDate ? " selected" : ""}`; cellEl.innerHTML = `<span class="jobpilot-calendar-number">${inMonth ? dayNumber : (cell < firstDay ? previousDays + dayNumber : dayNumber - daysInMonth)}</span>${dayJobs.slice(0,3).map(job => `<div class="jobpilot-calendar-job">${job.scheduled_time ? job.scheduled_time + " " : ""}${String(job.title || "Job").replace(/[<>]/g, "")}</div>`).join("")}${dayJobs.length > 3 ? `<div class="jobpilot-calendar-more">+${dayJobs.length-3} more</div>` : ""}`; cellEl.addEventListener("click", () => renderDetails(iso)); grid.appendChild(cellEl); }
...
async function updateUserMonthSnapshot(managementUser) {
  ...
  if (!stats) return;
  const weekCard = document.getElementById("jobpilot-user-week-jobs"); weekCard?.remove();
  ...
  const loadCalendar = async () => {
    try {
      ...
      const activeJobs = (data || []).filter(job => String(job.status || "").toLowerCase() !== "cancelled");
      monthCard.querySelector("strong").textContent = String(activeJobs.length); openMonthCalendar(activeJobs);
    } catch (error) { console.error("JobPilot user month calendar:", error); alert("The monthly work calendar could not be loaded. Please try again."); }
  };
...
async function applyDashboardRoleVisibility() {
  ...
  const monthJobsCard = cards[5];
  if (monthJobsCard) { monthJobsCard.style.display = managementUser ? "" : "none"; monthJobsCard.setAttribute("aria-hidden", managementUser ? "false" : "true"); }
...
function showQuoteRequestPanel() {
  const content = document.getElementById("pageContent"); if (!content || document.getElementById("jobpilot-user-quote-request")) return;
  const panel = document.createElement("div"); panel.id = "jobpilot-user-quote-request"; panel.className = "panel"; panel.style.marginTop = "20px";
  ...
  content.appendChild(panel); document.getElementById("jobpilot-request-quote-button")?.addEventListener("click", () => { if (typeof window.showQuoteRequestForm === "function") window.showQuoteRequestForm(); else console.error("JobPilot: quote request form is not available."); });
}

function enhanceDashboard() {
  const stats = document.querySelector(".stats"); if (!stats) return;
  hideDuplicateNavigation(); balanceTopNavigation(); hideUpcomingJobs(); makeDashboardCardsClickable();
  if (stats.dataset.snapshotCardsAdded !== "true") {
    stats.dataset.snapshotCardsAdded = "true";
    const todayJobsCard = document.createElement("div"); todayJobsCard.className = "stat-card"; todayJobsCard.innerHTML = `<div class="stat-icon">📅</div><div><span>Today's Jobs</span><strong>—</strong></div>`;
    const monthJobsCard = document.createElement("div"); monthJobsCard.className = "stat-card"; monthJobsCard.innerHTML = `<div class="stat-icon">🗓️</div><div><span>This Month's Jobs</span><strong>—</strong></div>`;
    todayJobsCard.style.gridColumn = "span 2"; monthJobsCard.style.gridColumn = "span 2";
    stats.appendChild(todayJobsCard); stats.appendChild(monthJobsCard);
  }
