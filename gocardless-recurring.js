...
async function openRecurringSetup(customer) {
  ...
  if (!Number.isInteger(amount) || amount < 1) { alert("Please enter a valid monthly amount."); return; }
  const day = window.prompt("Payment day (1-28)", "1");
  if (day === null) return;
  const dayOfMonth = Number(day);
  if (!Number.isInteger(dayOfMonth) || dayOfMonth < 1 || dayOfMonth > 28) { alert("Payment day must be between 1 and 28."); return; }
  const description = window.prompt("Description", "Monthly Direct Debit");
  if (description === null) return;
  const { data: { session } } = await supabase.auth.getSession();
  if (!session?.access_token) { alert("Please sign in again before setting up Direct Debit."); return; }
  try {
    const response = await fetch(CREATE_RECURRING_URL, { method: "POST", headers: { Authorization: `Bearer ${session.access_token}`, "Content-Type": "application/json" }, body: JSON.stringify({ customer_id: customer.id, amount, day_of_month: dayOfMonth, description: description.slice(0, 255) }) });
    const result = await response.json();
    if (!response.ok || !result.authorisation_url) throw new Error(result.error || "Unable to start Direct Debit setup");
