...
window.createInvoiceCheckout = async function (supabase, invoiceId) {
  ...
  try {
    if (button) { button.disabled = true; button.textContent = "Creating payment..."; }
    const { data: { session }, error: sessionError } = await supabase.auth.getSession();
    if (sessionError) throw new Error(sessionError.message);
    if (!session) throw new Error("You are not logged in.");
    const response = await fetch("https://qxoynttvipducubmczwl.supabase.co/functions/v1/stripe-checkout-v1", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` },
      body: JSON.stringify({ invoice_id: String(invoiceId), origin: window.location.origin })
    });
    ...
    console.error("JobPilot Stripe checkout error:", error);
