...
(() => {
  const state = { plan: "solo", company: null, jobs: [], customers: [], members: [], planner: null, tasks: [] };
  ...
  function renderList(content) {
    const cards=state.jobs.map(j=>`<div class="jp-planner-card" style="display:flex;justify-content:space-between;align-items:center;gap:12px"><div><strong>${esc(j.title||"Untitled job")}</strong><div class="jp-muted">${esc(customerName(j.customer_id))}${j.scheduled_date?` · ${esc(j.scheduled_date)}`:""}</div></div><button class="button secondary" data-plan-job="${j.id}" type="button">${j.plan_id?"Open plan":"Plan this job"}</button></div>`).join("");
    ...
    content.querySelectorAll("[data-plan-job]").forEach(b=>b.addEventListener("click",()=>openCreate(b.dataset.planJob)));
  }
