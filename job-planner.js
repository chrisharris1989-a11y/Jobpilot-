...
(() => {
  ...
  function renderList(content) {
    ...
    content.innerHTML=`<div class="jp-planner"><div class="page-actions"><div><h2>Job Planner</h2><p>Create a practical plan for an existing JobPilot job.</p></div><span class="jp-badge">${esc(state.plan.charAt(0).toUpperCase()+state.plan.slice(1))}</span></div><div class="jp-planner"><div class="jp-planner-card"><h3>Your jobs</h3><p class="jp-muted">Choose a job to start planning. You only enter the job information once.</p><div style="display:flex;flex-direction:column;gap:10px">${cards||'<div class="jp-muted">No jobs found.</div>'}</div></div></div></div>`;
  ...
  function renderPlanner(content){
    const p=state.planner; const done=state.tasks.filter(t=>t.status==="completed").length; const total=state.tasks.length; const allComplete=total>0&&done===total; const isCompleted=p.status==="completed"; const linkedJob=state.jobs.find(j=>String(j.id)===String(p.job_id));
    content.innerHTML=`<div class="jp-planner"><div class="page-actions"><div><h2>${esc(p.title)}</h2><p>${esc(customerName(linkedJob?.customer_id))} · ${esc(p.start_date||"No start date")} → ${esc(p.target_end_date||"No target")}</p></div><div class="jp-actions" style="margin:0"><button id="jp-back-list" class="button secondary" type="button">← Job Planner</button>${!isCompleted?`<button id="jp-add-task" class="button primary" type="button">+ Add task</button>`:""}</div></div><div class="jp-summary"><div class="jp-stat"><strong>${total}</strong><span class="jp-muted">Tasks</span></div><div class="jp-stat"><strong>${done}</strong><span class="jp-muted">Completed</span></div><div class="jp-stat"><strong>${total?Math.round(done/total*100):0}%</strong><span class="jp-muted">Progress</span></div></div>${allComplete||isCompleted?`<div class="jp-complete-panel"><div><strong>${isCompleted?"Plan finalised":"Plan ready to complete"}</strong><span class="jp-muted">${isCompleted?"This plan is complete and locked. It can no longer be edited.":"All planned tasks are complete. Mark the plan complete to make it final."}</span></div><div class="jp-actions" style="margin:0"><button id="jp-add-to-job" class="button secondary" type="button">Add plan to job</button>${!isCompleted?`<button id="jp-mark-complete" class="button primary" type="button">Mark plan complete</button>`:`<span class="jp-badge">Finalised</span>`}</div></div>`:`<div class="jp-complete-panel"><div><strong>Plan in progress</strong><span class="jp-muted">Complete all tasks to finish this plan and add the final plan to the job.</span></div></div>`}<div class="jp-planner-grid"><div class="jp-planner-card"><h3>Plan of work</h3><p class="jp-muted">${isCompleted?"Finalised plans are read-only.":"Edit tasks as the job develops."}</p><div class="jp-task-list">${state.tasks.map((t,i)=>taskHtml(t,i,isCompleted)).join("")}</div>${!isCompleted?`<div class="jp-actions"><button id="jp-save" class="button primary" type="button">Save changes</button></div>`:""}</div><div class="jp-planner-card"><h3>Plan features</h3><div class="jp-feature-list"><div class="jp-feature"><span>Basic planning</span><span class="jp-badge">Included</span></div><div class="jp-feature ${hasPlan("team")?"":"jp-locked"}"><span>👥 Assign team members</span><span class="jp-badge">Team</span></div><div class="jp-feature ${hasPlan("business")?"":"jp-locked"}"><span>🔗 Task dependencies</span><span class="jp-badge">Business</span></div><div class="jp-feature ${hasPlan("business")?"":"jp-locked"}"><span>📊 GANTT view</span><span class="jp-badge">Business</span></div><div class="jp-feature ${hasPlan("pro")?"":"jp-locked"}"><span>✨ Forecasting & optimisation</span><span class="jp-badge">Pro</span></div></div><div class="jp-timeline"><h3 style="margin-top:20px">Simple timeline</h3>${timelineHtml()}</div></div></div></div>`;
    content.querySelectorAll("[data-remove-task]").forEach(b=>b.addEventListener("click",()=>{if(state.planner?.status==="completed")return;state.tasks=state.tasks.filter(t=>String(t.id)!==String(b.dataset.removeTask));renderPlanner(content);}));
    content.querySelectorAll("[data-task-status]").forEach(b=>b.addEventListener("change",()=>{if(state.planner?.status==="completed"){b.checked=!b.checked;return;} b.checked?b.closest(".jp-task")?.classList.add("jp-task-done"):b.closest(".jp-task")?.classList.remove("jp-task-done");}));
  ...
  function taskHtml(t,i,readOnly=false){ const disabled=readOnly?" disabled":""; return `<div class="jp-task"><input type="checkbox" data-task-status="${t.id}" ${t.status==="completed"?"checked":""}${disabled} aria-label="Complete task"><div><div class="jp-task-title">${esc(t.title)}</div><div class="jp-task-meta">${t.start_date||"No date"} → ${t.end_date||"No date"}${t.predecessor_task_id?" · dependency":""}</div></div><input type="date" data-task-start="${t.id}" value="${esc(t.start_date||"")}"${disabled}><input type="date" data-task-end="${t.id}" value="${esc(t.end_date||"")}"${disabled}>${hasPlan("team")?`<select data-task-member="${t.id}"${disabled}><option value="">Unassigned</option>${state.members.map(m=>`<option value="${esc(m.user_id)}" ${String(m.user_id)===String(t.assigned_user_id)?"selected":""}>${esc(memberName(m.user_id))}</option>`).join("")}</select>`:`<span class="jp-badge">Solo</span>`}${!readOnly?`<button class="button secondary" data-remove-task="${t.id}" type="button">Remove</button>`:`<span class="jp-badge">Final</span>`}</div>`; }
  ...
  function addTask(){ if(state.planner?.status==="completed"){alert("This plan is final and cannot be edited.");return;} const id=`local-${Date.now()}`; state.tasks.push({id,title:"New task",description:null,duration_days:1,start_date:state.planner.start_date||today(),end_date:state.planner.start_date||today(),status:"pending",order_index:state.tasks.length,assigned_user_id:null,predecessor_task_id:null,_new:true}); render(); }

  async function saveChanges(){
    if(state.planner?.status==="completed"){alert("This plan is final and cannot be edited.");return;}
    const updates=[];
  ...
  async function markPlanComplete(){
    if(state.planner?.status==="completed"){return;}
