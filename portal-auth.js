...
async function callPin(action, payload) {
  ...
  const response = await fetch(PIN_FUNCTION_URL, {
    method: 'POST',
    headers,
    body: JSON.stringify({ action, ...payload, redirect_to: PORTAL_URL }),
  });
...
async function ensurePinAfterInvitation() {
  ...
  document.getElementById('portal-pin-setup-form').onsubmit = async event => {
    ...
    msg.textContent = '';
    try {
      await callPin('set', { pin });
      overlay.remove();
    } catch (error) {
      msg.textContent = error.message || 'Could not save your PIN.';
      button.disabled = false;
...
const observer = new MutationObserver(() => {
  ...
  installPinLogin(root);
});
observer.observe(document.body, { childList: true, subtree: true });

supabase.auth.onAuthStateChange(() => {
  setTimeout(() => ensurePinAfterInvitation().catch(() => {}), 250);
});

setTimeout(() => {
  const root = document.getElementById('portal-root');
  if (root) installPinLogin(root);
  ensurePinAfterInvitation().catch(() => {});
}, 100);
