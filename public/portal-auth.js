...
function showEmailLogin(root, email = '') {
  ...
  document.getElementById('portal-login-email').onsubmit = async event => {
    event.preventDefault(); const emailValue = document.getElementById('portal-email-fallback').value.trim(); const msg = document.getElementById('portal-msg-fallback');
    const { error } = await supabase.auth.signInWithOtp({ email: emailValue, options: { emailRedirectTo: PORTAL_URL } });
...
function installPinLogin(root, email = '') {
  ...
  document.getElementById('portal-pin-login').onsubmit = async event => {
    event.preventDefault(); const button = document.getElementById('portal-pin-submit'); const msg = document.getElementById('portal-pin-msg'); const emailValue = document.getElementById('portal-pin-email').value.trim(); const pin = document.getElementById('portal-pin').value.trim();
    button.disabled = true; button.textContent = 'Signing in…'; msg.textContent = '';
    try { const result = await callPin('login', { email: emailValue, pin }); if (!result.action_link) throw new Error('Could not create the portal session.'); window.location.href = result.action_link; }
    catch (error) { msg.textContent = error.message || 'Could not sign in.'; button.disabled = false; button.textContent = 'Sign in'; }
  };
...
async function ensurePinAfterInvitation() {
  ...
  const { data: account, error } = await supabase.from('customer_portal_accounts').select('id,pin_set_at,status').eq('user_id', session.user.id).eq('status', 'active').maybeSingle();
  if (error || !account || account.pin_set_at) return;
  if (document.getElementById('portal-pin-setup')) return;
  const overlay = document.createElement('div'); overlay.id = 'portal-pin-setup'; overlay.style.cssText = 'position:fixed;inset:0;background:rgba(15,23,42,.45);display:flex;align-items:center;justify-content:center;padding:20px;z-index:9999';
  overlay.innerHTML = `<div class="jp-portal-card" style="width:min(460px,100%);box-sizing:border-box"><h2>Set your portal PIN</h2><p class="jp-portal-muted">You have accepted your invitation. Create a 4-digit PIN so you can sign back in without requesting another email link.</p><form id="portal-pin-setup-form"><label>4-digit PIN<br><input id="portal-new-pin" type="password" inputmode="numeric" pattern="[0-9]{4}" minlength="4" maxlength="4" autocomplete="new-password" required style="width:100%;padding:12px;margin:6px 0 14px;box-sizing:border-box;letter-spacing:8px;text-align:center;font-size:20px"></label><label>Confirm PIN<br><input id="portal-new-pin-confirm" type="password" inputmode="numeric" pattern="[0-9]{4}" minlength="4" maxlength="4" autocomplete="new-password" required style="width:100%;padding:12px;margin:6px 0 14px;box-sizing:border-box;letter-spacing:8px;text-align:center;font-size:20px"></label><button id="portal-save-pin" class="jp-portal-button">Save PIN</button></form><p id="portal-pin-setup-msg" class="jp-portal-muted"></p></div>`;
  document.body.appendChild(overlay);
  document.getElementById('portal-pin-setup-form').onsubmit = async event => {
    event.preventDefault(); const pin = document.getElementById('portal-new-pin').value.trim(); const confirmPin = document.getElementById('portal-new-pin-confirm').value.trim(); const button = document.getElementById('portal-save-pin'); const msg = document.getElementById('portal-pin-setup-msg');
...
supabase.auth.onAuthStateChange(() => setTimeout(() => ensurePinAfterInvitation().catch(() => {}), 250));
setTimeout(() => { const root = document.getElementById('portal-root'); if (root) installPinLogin(root); ensurePinAfterInvitation().catch(() => {}); }, 100);
