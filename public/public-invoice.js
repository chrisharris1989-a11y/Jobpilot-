...
function renderInvoice(invoice, customer, business) {
  ...
  const address = [business?.address_line1, business?.city, business?.postcode]
    .filter(Boolean).map(escapeHtml).join("<br>");
  const customerAddress = [customer?.address_line1, customer?.city, customer?.postcode]
    .filter(Boolean).map(escapeHtml).join("<br>");
  ...
  container.innerHTML = `
    <div class="invoice-card">
      ...
      ${invoice.invoice_items?.length ? `<h3>Items</h3>${invoice.invoice_items.map(item => `<div class="invoice-row"><span>${escapeHtml(item.description || "")}</span><strong>${money(item.total)}</strong></div>`).join("")}` : ""}
      ...
      <div class="invoice-row" style="border-bottom:0;padding-top:20px"><strong class="invoice-total">Total</strong><strong class="invoice-total">${money(invoice.total)}</strong></div>
      ${paid ? `<div class="invoice-message">✅ This invoice has been paid.</div>` : `<button id="payButton" class="pay-button">💳 Pay Online</button>`}
  ...
  if (!paid) document.getElementById("payButton")?.addEventListener("click", createPayment);
}
