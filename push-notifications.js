...
export async function setupAdminPushNotifications(){
 ...
 if(Notification.permission==='granted'&&existing){const old=document.getElementById('jobpilot-push');if(old)old.remove();return;}
 ...
 b.onclick=async()=>{b.disabled=true;b.textContent='Enabling notifications…';try{const p=await Notification.requestPermission();if(p!=='granted')throw Error('Notification permission was not granted.');const reg=await navigator.serviceWorker.ready;let s=await reg.pushManager.getSubscription();if(!s)s=await reg.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:b64(VAPID_PUBLIC_KEY)});const j=s.toJSON();const {data,error}=await supabase.functions.invoke('admin-push',{body:{action:'subscribe',subscription:{endpoint:j.endpoint,keys:{p256dh:j.keys?.p256dh,auth:j.keys?.auth}},user_agent:navigator.userAgent}});if(error)throw error;if(!data?.success)throw Error(data?.error||'Subscription could not be saved.');b.remove();alert('Push notifications are now enabled on this device.')}catch(e){console.error(e);alert('Could not enable notifications: '+(e.message||e));b.disabled=false;b.textContent=ios()?'🔔 Enable iPhone notifications':'🔔 Enable phone notifications'}};
}
