...
(() => {
  ...
  const hasAccess=()=>BUSINESS_PLANS.has(state.plan);
  function styles(){
  ...
  async function load(){
    ...
    state.company=company; state.plan=String(company?.plan||"solo").toLowerCase(); if(!hasAccess()) return;
    const [{data:members,error:me},{data:jobs,error:je}]=await Promise.all([
    ...
    if(me)throw me;if(je)throw je;state.members=Array.isArray(members)?members:[];state.jobs=jobs||[];
  }
  ...
  const observer=new MutationObserver(()=>{const title=(document.getElementById("pageTitle")?.textContent||"").trim();if(title!=="Job Planner")return;const actions=document.querySelector(".jp-planner .page-actions");if(!actions||document.getElementById("jp-open-team-workload"))return;const b=document.createElement("button");b.id="jp-open-team-workload";b.className="button secondary";b.type="button";b.textContent="Team Workload";b.addEventListener("click",open);actions.appendChild(b);});
