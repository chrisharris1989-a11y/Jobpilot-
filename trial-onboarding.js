...
(function () {
  ...
  const PLANS = {
    core: { label: "Core", price: "£0/month", users: "1 user" },
  ...
  async function startSubscription(user){
    ...
    try{
      ...
      if(selectedPlan==="core"){
        ...
        if(message){message.textContent="Your free Core account is ready. Loading JobPilot...";message.style.color="#166534";message.style.display="block";}
  ...
  document.addEventListener("click",async(event)=>{
    ...
    if(data.session?.user)await startSubscription(data.session.user);else showError("Account created, but no login session was returned. Supabase email confirmation must be OFF for signup to continue directly.");
  },true);
})();
