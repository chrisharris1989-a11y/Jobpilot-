...
async function openAssignedRoute(event) {
  ...
  try {
    ...
    const [{data:jobs,error:jobsError},{data:customers,error:customersError}]=await Promise.all([
      supabase.from("jobs").select("id,customer_id,title,scheduled_date,scheduled_time,status,notes,assigned_user_id").eq("company_id",membership.company_id).eq("scheduled_date",date).eq("assigned_user_id",user.id),
