...
(function () {
  ...
  function copyField(source, card, suffix) {
    if (!source || card.querySelector(`#${source.id}-${suffix}`)) return;
    ...
    clone.addEventListener("input", () => { source.value = clone.value; });
  ...
  function populateAccountView(panel) {
    if (!accountView?.isConnected) return;

    ACCOUNT_FIELDS.forEach(([title, ids]) => {
      const card = accountView.querySelector(`[data-settings-account-card="${title}"]`);
      if (!card) return;
  ...
  function createAccountView(panel) {
    if (accountView?.isConnected) {
      populateAccountView(panel);
      return accountView;
    }
  ...
  function install(panel) {
    ...
    if (!document.getElementById("jobpilot-settings-account-fix-styles")) {
      ...
      style.textContent = `
        ...
        .settings-account-cards-clean { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 18px; }
        .settings-account-info-card { background: var(--surface, #fff); border: 1px solid var(--border, #e5e7eb); border-radius: var(--radius, 12px); box-shadow: var(--shadow, 0 2px 8px rgba(15, 23, 42, .04)); padding: 24px; }
        ...
        @media (max-width: 900px) { .settings-account-cards-clean { grid-template-columns: 1fr; } }
      `;
  ...
  function scan() {
    ...
    if (!panel) return;
    install(panel);
    // Settings fields can be populated after the panel is created.
    populateAccountView(panel);
  }
